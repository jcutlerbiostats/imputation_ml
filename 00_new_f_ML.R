...
# 
# # ADD sx3B to datB ----
# datB   <- cbind(syB,sx1B,sx2B,sx3B)
# datB2  <- as.data.frame(datB)
# 
# # ADD sx3A to datXA ----
# datXA  <- cbind(sx1A,sx2A,sx3A)
# datXA2 <- as.data.frame(datXA)
# 
# # The colnames for A are labelled with B's because we're gonna predict using A as new data 
# # (has to have same column names)
# colnames(datXA2) <- c('sx1B','sx2B','sx3B')
# 
# # ADD tibbles for datB2 & datXA2 ----
# datB2_tbl <- datB2 %>% as_tibble()
# datXA2_tbl <- datXA2 %>% as_tibble()
...
# # Fit model to sample B ----
# if (modeling_method        == "GAM") {
#   fit <- gam(syB ~ s(sx1B) + s(sx2B) + s(sx3B),
#              data = datB2)
# }
# 
# if (modeling_method == "LM") {
...
# if (modeling_method == "DEEP") {
#   
#   # ... ----
#   # * 1) model spec deep net ----
#   tabnet_spec <- tabnet(
#     epochs = 5,
#     batch_size = 256,
...
#     num_steps = tune(),         # Is this the number of layers? Different node number for each layer?
...
#   # * 3) resamples ----
#   set.seed(123)
#   dat_folds <- vfold_cv(datB2_tbl, v = 5)
#   
#   # * 4) tune_grid() ----
#   set.seed(123)
#   tabnet_grid <- tune_grid(
#     tabnet_workflow,
#     resamples = dat_folds,
#     grid = 20     # I've got 7 freaking parameters to tune. So ... how big should my grid be?
#   )
#   
#   # * 5) finalize_workflow() ----
#   final_tabnet_workflow <- tabnet_workflow %>% 
#     finalize_workflow(select_best(tabnet_grid,metric = "rmse"))
#   
#   # * 6) last_fit() ----
#   fit <- pull_workflow_spec(final_tabnet_workflow) %>% 
#     fit(syB ~ ., data = datB2_tbl)
# }
